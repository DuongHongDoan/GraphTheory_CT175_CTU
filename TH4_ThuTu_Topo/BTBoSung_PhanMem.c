...
int main () {
	...
	// 2. Doc danh sach cong viec de luu thanh do thi
	for (e=1; e<=n; e++) {
		scanf ("%d", &d[e]);
		do {
			//Doc cong viec dung truoc e
			scanf ("%d", &u);
			if (u>0)
				add_edge(&G, u, e);
		}while(u>0);
	...
	//Doc vao: cong viec thu may, duoc bat dau vao ngay nao
	int job, start;
	scanf ("%d%d", &job, &start);
	...
	//In ra thoi gian cv bat dau co nam trong thoi gian som nhat va muon nhat khong ?
//	if (start>t[job] && start<T[job])
//		printf("YES");
//	else 
//		printf("NO");

	//In ra cong viec then chot
	for (u=1; u<=n+2; u++) {
		if (t[u]==T[u])
			printf("%d\n", u);
	}
